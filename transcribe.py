#!/usr/bin/env python3
"""Transcribe podcast audio files to plain text using openai-whisper."""
...
from lib.config import AUDIO_DIR, WHISPER_MODEL, ensure_directories, transcript_path_for_episode
...
def process_episode(model, episode_filename: str, state: dict, force: bool = False) -> None:
    """Transcribe one episode if needed."""
    audio_path = AUDIO_DIR / episode_filename
    transcript_path = transcript_path_for_episode(episode_filename)
    episode = get_episode(state, episode_filename)

    if transcript_path.exists() and not force:
        episode["transcript_file"] = f"transcripts/{transcript_path.name}"
        episode["transcript_done"] = True
        print(f"Skipping {episode_filename} - transcript already exists")
        return
    ...
    transcript = clean_text(transcribe_file(model, audio_path))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Transcribe Take A Hike podcast episodes")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-transcribe even if transcript file already exists",
    )
    ...
    for index, episode_filename in enumerate(episodes, start=1):
        ...
        try:
            process_episode(model, episode_filename, state, force=args.force)
            save_state(state)
